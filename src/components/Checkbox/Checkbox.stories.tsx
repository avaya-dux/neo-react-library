...
const ControlledTemplate: Story<CheckboxProps> = ({checked: checkedProp = false, ...rest }: CheckboxProps) => {
...
const UncontrolledTemplate: Story<CheckboxProps> = ({defaultChecked, ...rest}: CheckboxProps) => {
  const ref = useRef(null)
  const onChange = useCallback(
  ...
  return <Checkbox ref={ref} defaultChecked={defaultChecked} onChange={onChange} {...rest} />
}
